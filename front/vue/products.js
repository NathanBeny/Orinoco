...
/*L'utilisateur à besoin d'un panier dans le localStorage
Vérifier si  panier existe dans  localStorage,
 sinon créer et envoyer dans  localStorage au premier chargement du site quelque soit la page*/
...
async function allProductsList() {
  ...
  produits.forEach((produit) => {
    ...
    //ajout attributs au balise pour la création css
...
addition = () => {
  ...
  if (JSON.parse(localStorage.getItem('userPanier')).length > 0) {
    ...
    //Pour chaque produit du panier, on créé  ligne avec  nomprix
